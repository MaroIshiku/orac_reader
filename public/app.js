...
$("#partForm").onsubmit = async (event) => {
  event.preventDefault(); const form = event.target; const data = Object.fromEntries(new FormData(form)); const payload = { chapterId: data.chapterId, part: Number(data.part), releasedAt: data.releasedAt, partTitle: data.partTitle, tldr: data.tldr, content: data.content, hidden: data.hidden === "on" }; const isNew = !form.dataset.partId;
