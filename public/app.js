...
const state = { books: [], links: [], admin: false, book: null, part: null, fontSize: Number(localStorage.getItem("oracle-font-size")) || 19, view: localStorage.getItem("oracle-view") || "scroll", theme: localStorage.getItem("oracle-theme") || "paper" };
...
async function load() {
  const data = await api("/api/library"); state.books = data.books; state.links = data.links; state.admin = data.admin;
...
function renderLibrary(query = "") {
  ...
  const books = state.books.filter((book) => !search || [book.title, book.description, book.tldr, ...allParts(book).flatMap(({chapter, part}) => [chapter.title, part.title, part.content])].join(" ").toLocaleLowerCase("de").includes(search));
  $("#emptyLibrary").hidden = books.length > 0;
  $("#bookGrid").innerHTML = books.map((book, index) => {
    const displayIndex = String(state.books.indexOf(book) + 1).padStart(2,"0"); const parts = allParts(book); const recentPartId = getHistory().find((entry) => entry.bookId === book.id)?.partId; const target = parts.find(({part}) => part.id === recentPartId) || parts[0]; const allRead = parts.length && parts.every(({part}) => isRead(book.id, part.id)); const status = book.status !== "published" ? `<span class="status-badge">${book.status === "draft" ? "ENTWURF" : "GEPLANT"}</span>` : "";
    return `<article class="book-card" tabindex="0" role="link" data-open-book="${escapeHtml(book.id)}" data-open-part="${escapeHtml(target?.part.id || "")}" data-index="${displayIndex}"><div class="generated-cover"><img src="/oracle-logo.png" alt=""><span>ORACLE FILE</span><strong>${displayIndex}</strong></div><div class="book-number">CHRONIK ${displayIndex} ${status}</div><h3>${escapeHtml(book.title)}</h3><p>${escapeHtml(book.description)}</p><footer><span>${partCount(book)} ${partCount(book) === 1 ? "Teil" : "Teile"} · ${formatDate(book.updatedAt)}</span>${target ? `<button data-toggle-book-read="${escapeHtml(book.id)}">${allRead ? "✓ Gelesen" : "○ Ungelesen"}</button>` : ""}</footer></article>`;
  }).join("");
...
function renderHomeShelves() {
  ...
  $("#recentShelf").hidden = !recent.length; $("#recentBooks").innerHTML = recent.map(({book,chapter,part}) => `<button class="shelf-item" data-shelf="${escapeHtml(book.id)}:${escapeHtml(part.id)}"><span class="shelf-number">${chapter.number}.${part.number}</span><span><b>${escapeHtml(part.title)}</b><small>${escapeHtml(book.title)}</small></span></button>`).join("");
  ...
  $("#newBooks").innerHTML = newest.map((book) => { const {chapter,part} = allParts(book)[0]; return `<button class="shelf-item" data-shelf="${escapeHtml(book.id)}:${escapeHtml(part.id)}"><span class="shelf-number">${chapter.number}.${part.number}</span><span><b>${escapeHtml(book.title)}</b><small>${escapeHtml(part.title)}</small></span></button>`; }).join("");
...
function openPart(book, chapter, part) {
  ...
  $("#storyDescription").textContent = book.description; $("#storyPart").textContent = `Kapitel ${chapter.number} · Teil ${part.number}`;
  ...
  $("#chapterList").innerHTML = book.chapters.map((item) => `<li><small class="chapter-group">KAPITEL ${item.number} · ${escapeHtml(item.title)}</small>${item.parts.map((candidate) => `<button class="${candidate.id === part.id ? "active" : ""} ${isRead(book.id, candidate.id) ? "read" : ""}" data-open-part="${escapeHtml(candidate.id)}"><small>${item.number}.${candidate.number}</small><span>${escapeHtml(candidate.title)}</span></button>`).join("")}</li>`).join("");
...
function fillBook(form, book) { field(form,"title").value = book?.title || ""; field(form,"description").value = book?.description || ""; field(form,"tldr").value = book?.tldr || ""; field(form,"status").value = book?.status || "draft"; field(form,"publishAt").value = book?.publishAt ? new Date(book.publishAt).toISOString().slice(0,16) : ""; }
function editPart(bookId = "", partId = "") {
  ...
  field(form,"chapter").value = found?.chapter.number ?? book?.chapters.at(-1)?.number ?? 0; field(form,"chapterTitle").value = found?.chapter.title || (book?.chapters.at(-1)?.title || ""); field(form,"part").value = found?.part.number || ((book?.chapters.at(-1)?.parts.at(-1)?.number || 0) + 1); field(form,"partTitle").value = found?.part.title || ""; field(form,"content").value = found?.part.content || "";
...
$("#pagePrev").onclick = () => turnPage(-1); $("#pageNext").onclick = () => turnPage(1); $("#shareButton").onclick = shareCurrent;
...
$("#bookForm").onsubmit = async (event) => {
  event.preventDefault(); const form = event.target; const data = Object.fromEntries(new FormData(form)); const bookPayload = { title: data.title, description: data.description, tldr: data.tldr, status: data.status, publishAt: data.publishAt };
...
$$('[data-admin-tab]').forEach((button) => button.onclick = () => { $$('[data-admin-tab]').forEach((item) => item.classList.toggle("active", item === button)); $("#booksAdmin").hidden = button.dataset.adminTab !== "books"; $("#linksAdmin").hidden = button.dataset.adminTab !== "links"; });
