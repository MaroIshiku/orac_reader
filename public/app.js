...
function renderAdmin() {
  ...
  list.onclick = async (event) => {
    ...
    try {
      ...
      if (button.dataset.deleteBook) { const book = state.adminBooks.find((item) => item.id === button.dataset.deleteBook); if (book && confirm(`„${book.title}“ samt Kapiteln und Teilen endgültig löschen?`)) { await api(`/api/admin/books/${encodeURIComponent(book.id)}`, { method: "DELETE" }); closeEditors(); await load(); toast("Archiveintrag gelöscht"); } }
