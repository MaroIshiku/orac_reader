...
test("die vier ursprünglichen Oracle-Akten werden verlustfrei zu einem Buch migriert", async () => {
  ...
  try {
    ...
    assert.deepEqual(oracle.chapters.map((chapter) => chapter.number), [0, 1, 2, 3]);
    assert.deepEqual(oracle.chapters.map((chapter) => chapter.parts[0].content), ["Inhalt 0", "Inhalt 1", "Inhalt 2", "Inhalt 3"]);
